          ...
          <div>
            ...
            <div className="flex border-b border-gray-200 mb-8">
              <button 
                ...
                className={`px-5 py-2 bg-transparent border-none ${activeTab === 'overview' ? 'border-b-3 border-primary font-bold' : 'font-normal'} cursor-pointer`}
              >
                Overview
              </button>
              <button 
                onClick={() => setActiveTab('analytics')}
                className={`px-5 py-2 bg-transparent border-none ${activeTab === 'analytics' ? 'border-b-3 border-primary font-bold' : 'font-normal'} cursor-pointer`}
              >
                Analytics
              </button>
              <button 
                onClick={() => setActiveTab('settings')}
                className={`px-5 py-2 bg-transparent border-none ${activeTab === 'settings' ? 'border-b-3 border-primary font-bold' : 'font-normal'} cursor-pointer`}
              >
                Settings
              </button>
            </div>
            ...
            {activeTab === 'overview' && (
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                  {tokens.map(token => (
                    <div key={token.id} className="overflow-hidden rounded-xl border-4 border-black shadow-md">
                      ...
                      <div className="p-5">
                        <h3 className="mb-2">{token.name} ({token.symbol})</h3>
                        ...
                        <div className="flex justify-between mb-5">
                          <div className="text-center">
                            <div className="font-bold text-lg">{token.followers}</div>
                            <div className="text-xs text-gray-600">Followers</div>
                          </div>
                          <div className="text-center">
                            <div className="font-bold text-lg">{token.engagement}%</div>
                            <div className="text-xs text-gray-600">Engagement</div>
                          </div>
                          <div className="text-center">
                            <div className="font-bold text-lg">{token.messages}</div>
            ...
            {activeTab === 'analytics' && (
              <div className="border-8 border-black rounded-3xl bg-white shadow-[-3px_3px_0_0_#1f2024] p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                  ...
                  <div className="p-5 bg-gray-50 rounded-lg text-center">
                    <div className="text-2xl font-bold mb-1">
                      {Math.round(tokens.reduce((sum, token) => sum + token.engagement, 0) / tokens.length)}%
                    </div>
                ...
                <p className="text-center text-gray-600">
                  Detailed analytics coming soon! Check back for more insights about your AI character's performance.
                </p>
              </div>
            )}
